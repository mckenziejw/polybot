"""
replay_cleanrl.py — Market replay viewer for CleanRL PPO+LSTM models.

Usage:
    python replay_cleanrl.py

Then open http://localhost:5006 in your browser.

Optional args:
    python replay_cleanrl.py
        --train-dir  data/telonex_15m_100ms
        --test-dir   data/telonex_15m_100ms_test
        --model      models/checkpoints/ckpt_0001000000.pt
        --normalizer models/checkpoints/vecnorm_0001000000.npz
        --resolution data/15m_resolutions.json
        --port       5006
"""
...
parser.add_argument('--train-dir',   default='data/telonex_15m_100ms')
parser.add_argument('--test-dir',    default='data/telonex_15m_100ms_test')
parser.add_argument('--model',       default='models/checkpoints/ckpt_0001000000.pt')
parser.add_argument('--normalizer',  default='models/checkpoints/vecnorm_0001000000.npz')
parser.add_argument('--btc-path',    default='data/btc_quotes/btcusdt_quotes.parquet')
parser.add_argument('--resolution',  default='data/15m_resolutions.json')
...
from polymarket_env import PolymarketEnv, OBS_DIM, N_ACTIONS
from train_cleanrl import Agent, VecNormalize
...
agent = Agent(
    ...
    mlp_hidden = saved_args.get('mlp_hidden', 256),
...
print(f'Loading VecNormalize stats from {NORMALIZER_PATH}...')
# We use a single-env VecNormalize just for obs normalization during replay
vec_norm = VecNormalize(
    num_envs  = 1,
    obs_shape = (OBS_DIM,),
    gamma     = saved_args.get('gamma', 0.99),
)
vec_norm.load(str(NORMALIZER_PATH))
vec_norm.training = False   # freeze stats — don't update during replay
...
def run_episode(slug: str, book_dir: Path) -> dict:
    """
    Run one full episode with the loaded CleanRL agent.
    Returns a records dict suitable for chart rendering.
    """
    env = PolymarketEnv(
        book_dir        = book_dir,
        btc_path        = BTC_PATH,
    ...
    obs_raw, _ = env.reset()
    obs = torch.tensor(
        vec_norm.normalize_obs(obs_raw[np.newaxis]),   # add batch dim
        dtype=torch.float32,
        device=device,
    )

    # LSTM hidden state: (num_layers=1, batch=1, hidden_size)
    lstm_state = (
    ...
    terminal_reward = 0.0
    N = 5  # order book levels to record

    while True:
        # --- Record pre-step market state ---
        pre_idx     = min(env._step_idx, len(env._timestamps) - 1)
        pre_ts      = env._timestamps[pre_idx]
        pre_yes_row = env._yes_df.iloc[pre_idx]
        pre_no_row  = env._no_df.iloc[pre_idx]
        pre_yes_mid = float(pre_yes_row.get('mid_price', 0.5))
        pre_no_mid  = float(pre_no_row.get('mid_price',  0.5))

        yes_book = (
            [float(pre_yes_row.get(f'bid_price_{i}', 0)) for i in range(1, N+1)],
            [float(pre_yes_row.get(f'bid_size_{i}',  0)) for i in range(1, N+1)],
            [float(pre_yes_row.get(f'ask_price_{i}', 0)) for i in range(1, N+1)],
            [float(pre_yes_row.get(f'ask_size_{i}',  0)) for i in range(1, N+1)],
        )
        no_book = (
            [float(pre_no_row.get(f'bid_price_{i}', 0)) for i in range(1, N+1)],
            [float(pre_no_row.get(f'bid_size_{i}',  0)) for i in range(1, N+1)],
            [float(pre_no_row.get(f'ask_price_{i}', 0)) for i in range(1, N+1)],
            [float(pre_no_row.get(f'ask_size_{i}',  0)) for i in range(1, N+1)],
        )

        # --- Get action masks and run model ---
        masks = torch.tensor(
            env.action_masks()[np.newaxis],   # (1, N_ACTIONS)
            dtype=torch.bool,
            device=device,
        )
        ...
        with torch.no_grad():
            action, _, _, _, lstm_state = agent.get_action_and_value(
                obs, lstm_state, done, action_masks=masks
            )

        action_int = int(action.item())

        # --- Record pre-step state, post-step financials ---
        records['timestamps'].append(int(pre_ts))
        records['yes_mid'].append(pre_yes_mid)
        records['no_mid'].append(pre_no_mid)
        records['actions'].append(action_int)
        records['yes_position'].append(env._yes_position)
        records['no_position'].append(env._no_position)
        records['yes_book'].append(yes_book)
        records['no_book'].append(no_book)

        # --- Step environment ---
        obs_raw, reward, terminated, truncated, info = env.step(action_int)
        done_bool = terminated or truncated

        # Update obs and done flag for next LSTM step
        obs = torch.tensor(
            vec_norm.normalize_obs(obs_raw[np.newaxis]),
            dtype=torch.float32,
            device=device,
        )
        done = torch.tensor([1.0 if done_bool else 0.0], device=device)

        records['step_rewards'].append(float(reward))
        # Post-step financials reflect the action just taken
        records['bankroll'].append(env._bankroll)
        records['realized_pnl'].append(env._realized_pnl)
        records['fees_paid'].append(env._fees_paid)
    ...
    records['yes_resolved']    = env._yes_resolved
    records['no_resolved']     = env._no_resolved
    records['slug']            = slug
...
# ---------------------------------------------------------------------------
# Chart builders (unchanged from original replay.py)
# ---------------------------------------------------------------------------
...
def make_price_chart(records):
    ...
    def _marker_df(indices):
        rows = []
        for i in indices:
            a     = acts[i]
            name  = ACTION_NAMES[a]
            price = (ymd[i] if a in (1, 2) else
                     nmd[i] if a in (3, 4) else
    ...
    for df, label, color, marker in [
        (_marker_df(buy_yes_idx), '▲ Buy Yes (S=small, L=large)', 'limegreen', 'triangle'),
        (_marker_df(buy_no_idx),  '▼ Buy No (S=small, L=large)',  'tomato',    'inverted_triangle'),
        (_marker_df(sell_idx),    '■ Sell (close position)',       'gold',      'square'),
    ]:
...
def make_book_chart(records, step):
    ...
    def cumulative_depth(bid_p, bid_s, ask_p, ask_s):
        ...
        cum_bid, cum_ask, c = [], [], 0
        for p, s in bids:
            c += s; cum_bid.append((p, c))
        c = 0
        for p, s in asks:
            c += s; cum_ask.append((p, c))
...
def refresh_charts():
    ...
    book_pane.object     = (make_book_chart(records, player.value)
                            if mode_select.value == 'Scrub' else hv.Curve([]))


app = pn.Column(
    pn.pane.Markdown('# Market Replay Viewer  *(CleanRL PPO+LSTM)*'),
    pn.Row(
        pn.Column(
            split_select,
            slug_select,
...
pn.serve(app, port=args.port, show=True, title='Market Replay (CleanRL)')