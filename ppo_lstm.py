...
def make_env(env_id, idx, capture_video, run_name):
    def thunk():
        env = PolymarketEnv(
            ...
            resolution_path=Path(args.resolution_path)
        )
...
class Agent(nn.Module):
    def __init__(self, obs_dim, action_dim, lstm_hidden=256):
        super().__init__()
        ...
        self.lstm = nn.LSTM(256, lstm_hidden)
    ...
    def get_action_and_value(self, x, lstm_state, done, action=None, action_masks=None):
...
if __name__ == "__main__":
    ...
    action_masks = torch.zeros((num_steps, num_envs, action_dim), dtype=torch.bool).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(num_steps):
            ...
            masks = torch.tensor(
                np.array([env.action_masks() for env in envs.envs]),
                dtype=torch.bool
            ).to(device)
